...
{   
    ...
    $this->Cell(110,3,utf8_decode('LABORATORIO BLESSING / '.$remetente['nome']),'LRB',0,'L');//VARIAVEL DE NOME / RAZÃO SOCIAL
    $this->Cell(40,3,utf8_decode('18.313.334/0001-80'),'BR',0,'L');//VARIAVEL DE CNPJ / CPF
    ...
    $sql = "SELECT u.nome as nome, u.logradouro as logradouro, u.bairro as bairro, u.cep as cep, u.uf as uf, u.cidade as cidade, u.fone as tel, u.numero as numero
    ...
    $this->Ln();
    $this->SetFont('Arial','B',6);
    $this->Cell(110,3,utf8_decode('LABORATORIO BLESSING / '.$destinatario['nome']),'LRB',0,'L');//VARIAVEL DE NOME / RAZÃO SOCIAL
    $this->Cell(40,3,utf8_decode('18.313.334/0001-80'),'BR',0,'L');//VARIAVEL DE CNPJ / CPF
    $this->Cell(40,3,utf8_decode('______/______/______'),'BR',0,'L');
    $this->Ln();
...
function Footer()
{
    ...
    $this->Cell(0,8,'Pag: '.$this->PageNo().'/{nb}',0,0,'R');
        ...
        $adminCompras = intval($_SESSION['adm_compras']);
        ...
        $sql = "SELECT SUM(qtd_slt) as total FROM `estoque_item_requisicao` where status = 563 && registro = $req";
        $sql = mysqli_query( $link, $sql );
        ...
        foreach ($idRegistro as $key => $value) {
        $sql = "UPDATE estoque_item_requisicao 
        SET status = 565, imp = 1, data = $data 
        where id = $value";
        $sql = mysqli_query( $link, $sql );
        }
